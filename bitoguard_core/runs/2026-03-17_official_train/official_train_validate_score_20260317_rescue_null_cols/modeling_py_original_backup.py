...
def fit_catboost(
    ...
    positives = max(1, int(y_train.sum()))
    negatives = max(1, len(y_train) - positives)
    # When focal_gamma > 0, focal loss already handles class imbalance.
    # Applying class_weights on top double-counts the correction and causes
    # gradient instability over many iterations, collapsing Base B probabilities.
    class_weights = [1.0, negatives / positives] if focal_gamma == 0.0 else [1.0, 1.0]
    # focal_gamma > 0 activates CatBoost focal loss (Lin et al. 2017).
    # focal_gamma=2.0 focuses gradient on hard boundary cases — beneficial for
    # severe class imbalance. Default 0.0 preserves standard Logloss behavior.
    _loss_function = (
        f"Logloss:focal_gamma={focal_gamma}" if focal_gamma > 0.0 else "Logloss"
    )
